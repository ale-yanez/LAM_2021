...
out1 <- read.admb("../ModBase_M/output/Lam")
out2 <- read.admb("../ModBase_cvY/output/Lam")
out3 <- read.admb("../ModBase_Funcional/output/Lam")
out4 <- read.admb("../ModBase/output/Lam")
...
p1_2 <-  ggplot(NULL, aes(x=yrs)) +
  geom_point(aes(y= obsD, colour="Observado"), size = 2, shape = 21) +
  geom_line(aes(y= predD, colour="M")) +
  geom_line(aes(y= predD2, colour="cv_Y")) +
  geom_line(aes(y= predD3, colour="Funcional")) +
  geom_line(aes(y= predD4, colour="Base")) +
  geom_errorbar(aes(ymin = obsD95i, ymax = obsD95s), colour='black', width=0.6) +
  scale_colour_manual(name='', values=c('M'='red', 'cv_Y'='gold2', 'Funcional'='green', 'Base'='royalblue3', 'Observado'='black'), guide='legend') +
...
p2 <-  ggplot(NULL, aes(x=yrs)) +
  geom_point(aes(y= obsC, colour="CPUE Obs"), size = 2, shape = 21) +
  geom_line(aes(y= predC, colour="M")) +
  geom_line(aes(y= predC2, colour="cv_Y")) +
  geom_line(aes(y= predC3, colour="Funcional")) +
  geom_line(aes(y= predC4, colour="Base")) +
  geom_errorbar(aes(ymin = obsC95i, ymax = obsC95s), colour='black', width=0.6) +
  #scale_colour_manual(name='', values=c('CPUE Est'='royalblue3', 'CPUE Obs'='black'), guide='legend') +
  scale_colour_manual(name='', values=c('M'='red', 'cv_Y'='gold2', 'Funcional'='green', 'Base'='royalblue3', 'CPUE Obs'='black'), guide='legend') +
  guides(colour = guide_legend(override.aes = list(linetype=c(1,1,1,1,0), shape=c(NA,NA,NA,NA, 21)))) +
...
p3 <-  ggplot(NULL, aes(x=yrs)) +
  geom_point(aes(y= obsS, colour="Crucero Obs"), size = 2, shape = 21) +
  geom_line(aes(y= predS, colour="M")) +
  geom_line(aes(y= predS2, colour="cv_Y")) +
  geom_line(aes(y= predS3, colour="Funcional")) +
  geom_line(aes(y= predS4, colour="Base")) +
  geom_errorbar(aes(ymin = obsS95i, ymax = obsS95s), colour='black', width=0.6) +
  #scale_colour_manual(name='', values=c('Crucero Est'='royalblue3', 'Crucero Obs'='black'), guide='legend') +
  scale_colour_manual(name='', values=c('M'='red', 'cv_Y'='gold2', 'Funcional'='green', 'Base'='royalblue3', 'Crucero Obs'='black'), guide='legend') +
  guides(colour = guide_legend(override.aes = list(linetype=c(1,1,1,1,0), shape=c(NA,NA,NA,NA, 21)))) +
...
std1 <- read.table('../ModBase_M/output/Lam.std', header = T, sep = '', na='NA', fill = T)
std2 <- read.table('../ModBase_cvY/output/Lam.std', header = T, sep = '', na='NA', fill = T)
std3 <- read.table('../ModBase_Funcional/output/Lam.std', header = T, sep = '', na='NA', fill = T)
std4 <- read.table('../ModBase/output/Lam.std', header = T, sep = '', na='NA', fill = T)
...
p10 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  geom_line(aes(y = BT_est4, colour = 'Base', linetype = 'Base')) +
  ...
  scale_color_manual(name = '',
                     values = c('red', 'gold2', 'green', 'royalblue3'),
                     limits = c('M', 'cv_Y', 'Funcional', 'Base'),
                     breaks = c('M', 'cv_Y', 'Funcional', 'Base')) +
...
p10 <- p10 + theme_bw() + 
  ...
  scale_x_continuous(breaks=round(seq(min(yrs), 2020, by = 3),1))
...
p11 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  geom_line(aes(y = BD_est4, colour = 'Base', linetype = 'Base')) +
  #geom_ribbon(data=NULL, aes(ymin=BT1_lwr, ymax=BT1_upr), fill = 'grey60', alpha = 0.4) + 
  #geom_line(aes(y = c(rep(1,length(yrs))), colour = 'Brms', linetype = '')) +
  scale_color_manual(name = '',
                     values = c('red', 'gold2', 'green', 'royalblue3'),
...
ggsave(plot_B, filename = "../R_contrast_Sens/figures/VarPop2_Biom.png", width=8, height=6.5, dpi=300)
