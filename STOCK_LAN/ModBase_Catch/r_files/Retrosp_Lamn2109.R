...
#setwd('../input/')
file.copy('../input/STOCK_LAN.dat', '../Retrospectivo/')
...
figretro <- ggplot(matretro) + geom_line(aes(x = year, y = value, color = Periodo)) + 
  xlab('Años') + scale_x_continuous(breaks=round(seq(min(years), max(years), by = 3),1)) +
...
ggsave(figretro, filename = "../figures/Fig1_Retro.pdf", width=7, height=5.5, dpi=300)
