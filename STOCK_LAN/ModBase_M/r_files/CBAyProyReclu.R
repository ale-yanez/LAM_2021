...
CBApstd <-subset(std1,name=='CBA')$std[3]
 ...
 write.table(tCBA, '../tables/CBA.txt', append = FALSE, sep = " ", dec = ".", row.names = TRUE, col.names = TRUE)
  
 
# Proyección CBA bajo distintos F ####

dir.create(file.path('../','CBA_Proy'))
file.copy('../input/stock_LAN.dat', '../CBA_Proy')
file.copy('../output/Lam.rep', '../CBA_Proy')
file.copy('../runfolder/Lam', '../CBA_Proy')
...
#setwd(dir.3)
data        <- lisread('../input/stock_LAN.dat')
...
# EXPLORATORIO DE Fsq ####
setwd('../CBA_Proy/')
system('cat ../output/Lam.rep')
# 0.442051

getwd();dir()
system('./Lam -ind stock_LAN.dat')

 
# Run Model con NUMERO Y TASA DE PROYECCIÓN PBRs ESCENARIOS ####

# escenario 1: Ro*1
Tasa_bdpr <- c(0.3,0.4,0.45, 0.442051)
new_yr_proy <- 10
l_npbr <- length(Tasa_bdpr)
dat$npbr   <- l_npbr
dat$Tasa_bdpr <- Tasa_bdpr
dat$yr_proy <- new_yr_proy
...
ggsave(p1_2, filename = "Figs_Proyec/Fig1_2_2BD.png", width=8, height=6.5, dpi=300)
...
p1_3 <- ggplot(BD_Proy, aes(x = yrs)) + 
  ...
  xlab('Años') + scale_x_continuous(breaks=round(seq(min(yrs), 2030, by = 1),1)) +
  ylab('Biomasa Desovante (t)') + ylim(3500,5000) +#scale_y_continuous(breaks=round(seq(0, 4500, by = 500),1)) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text=element_text(size=10)) +
  theme(legend.position = 'bottom')  + theme(legend.title=element_blank()) + theme(legend.text = element_text(size = 10)) + 
  theme(axis.title.x = element_text(size = 12)) + 
  theme(axis.title.y = element_text(size = 12)) 
...
p1_3
ggsave(p1_3, filename = "Figs_Proyec/Fig1_3BD.png", width=8, height=6.5, dpi=300)
...
p2_2 <- ggplot(C_Proy, aes(x = yrs)) + 
  geom_line(aes(y = F45, colour = 'F45', linetype = 'F45')) +
  geom_line(aes(y = Fsq, colour = 'Fsq', linetype = 'Fsq')) + 
  geom_line(aes(y = c(rep(CBA[5],10)), colour = 'CBA', linetype = 'CBA')) +
  scale_color_manual(name = '',
                     values = c('green4', 'red', 'royalblue3'),
...
p2_2_1 <- ggplot(C_Proy, aes(x = yrs)) + 
  ...
  xlab('Años') + scale_x_continuous(breaks=round(seq(min(yrs), 2030, by = 1),1)) +
  ylab('Captura (t)') + ylim(1000, 3000) + # scale_y_continuous(breaks=round(seq(min(predC), 3, by = 0.5),1)) + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text=element_text(size=11)) +
  theme(legend.position = 'bottom')  + theme(legend.title=element_blank())


p2_2_1
ggsave(p2_2_1, filename = "Figs_Proyec/Fig2_2_1_BD.png", width=8, height=6.5, dpi=300)
