...
p1_2 <-  ggplot(NULL, aes(x=yrs)) +
  ...
  xlab('Años') + scale_x_continuous(breaks=round(seq(min(yrs), 2020, by = 3),1)) +
  ylab('Desembarque (t)') + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text=element_text(size=8)) +
  theme(legend.position = 'bottom')  + theme(legend.title=element_blank())
...
# Machos Flota
 ...
 df_mcruobs <- df_mcruobs[-c(1:13, 24), ]
 ...
 df_mcruest <- df_mcruest[-c(1:13, 24), ]
 ...
 df_hcruobs <- df_hcruobs[-c(1:13, 24), ]
 ...
 df_hcruest <- df_hcruest[-c(1:13, 24), ]
