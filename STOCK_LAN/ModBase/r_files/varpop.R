...
p8 <- p8 + theme_bw() + 
  ...
  theme(legend.position = 'bottom') + ylab('Reclutas x 10^6') + xlab('Años') + scale_x_continuous(breaks=round(seq(min(yrs), 2020, by = 3),1))
...
p12 <- ggplot(data = NULL, aes(x = yrs)) + 
  ...
  annotate("text", x=1989, y=0.29, label="M") +
...
ggsave(p12, filename = "../figures/VarPop3_Fh.png", width=8, height=5.5, dpi=300)
