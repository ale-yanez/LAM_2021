...
# Confidence Intervals
 ...
 obsLm95i   <- Lobs_mf*exp(-1.96*cvLf); 
 obsLm95s   <- Lobs_mf*exp(1.96*cvLf);
...
p5 <-  ggplot(df_flo, aes(x=yrs)) +
  ...
  #geom_errorbar(aes(ymin = obsLm95i, ymax = obsLm95s), colour='black', width=0.5) +
  scale_colour_manual(name='', values=c('Talla Media Est.'='royalblue3', 'Talla Media Obs.'='black'), guide='legend') +
  guides(colour = guide_legend(override.aes = list(linetype=c(1,0), shape=c(NA, 21)))) +
  
  xlab('Años') + scale_x_continuous(breaks=round(seq(min(yrs), 2020, by = 5),1)) +
  ylab('LC machos (mm)') + ylim(25, 45) + 
...
p1_2 <-  ggplot(df_flo, aes(x=yrs)) +
  geom_point(aes(y= Lobs_mf, colour="Observado"), size = 2, shape = 21) +
  geom_line(aes(y= Lest_m, colour="Estimado")) +
  #geom_errorbar(aes(ymin = obsLm95i, ymax = obsLm95s), colour='black', width=0.6) +
  scale_colour_manual(name='', values=c('Estimado'='royalblue3', 'Observado'='black'), guide='legend') +
  guides(colour = guide_legend(override.aes = list(linetype=c(1,0), shape=c(NA, 21)))) +
...
df_Sel <- data.frame(c(1:11), out1$Sflom_age[1,], out1$Sfloh_age[1,], out1$Scrum_age[19,], out1$Scruh_age[19,])
...
df_mflobs <- df_mflobs[-c(1:12, 17:20), ]
...
df_mfloest <- df_mfloest[-c(1:12, 17:20), ]
...
ggsave(p1, filename = "../figures/Fig2_TallasM_flo.png", width=8.5, height=6.5, dpi=300)
...
df_hflobs <- df_hflobs[-c(1:12, 17:20), ]
...
df_hfloest <- df_hfloest[-c(1:12, 17:20), ]
...
ggsave(p2, filename = "../figures/Fig3_TallasH_flo.png", width=8.5, height=6.5, dpi=300)
...
df_mcruobs <- df_mcruobs[-c(1:15, 26), ]
...
df_mcruest <- df_mcruest[-c(1:15, 26), ]
...
ggsave(p3, filename = "../figures/Fig4_TallasM_cru.png", width=8.5, height=6.5, dpi=300)
...
df_hcruobs <- df_hcruobs[-c(1:15, 26), ]
...
df_hcruest <- df_hcruest[-c(1:15, 26), ]
...
ggsave(p4, filename = "../figures/Fig5_TallasH_cru.png", width=8.5, height=6.5, dpi=300)
